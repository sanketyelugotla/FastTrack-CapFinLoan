...
# Uvicorn spawns a main process and worker processes — kill all
...
if ($uvicornProcs) {
    ...
    Write-Host "✓ ChatbotService stopped." -ForegroundColor Green
} else {
    # Fallback: kill any uvicorn process on port 5026
    $portProc = Get-NetTCPConnection -LocalPort 5026 -ErrorAction SilentlyContinue |
